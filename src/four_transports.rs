#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `evgl-api`.
//!
...
//! to evento-globolo — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `evgl-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `evgl-api` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `evgl-api` cluster |
...
//! `evgl-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `EVGL_API_URL`, `EVGL_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "EVGL";
...
pub const SERVICE_SLUG: &str = "evgl";
