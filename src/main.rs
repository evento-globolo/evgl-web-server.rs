...
fn layout(content: Markup) -> Markup {
    html! {
        ...
        html lang="en" {
            ...
            body {
                header { h1 { "Evento Globolo" } p { "A global events operating system combining event discovery, publishing, RSVP, ticketing, community, venue, and organizer workflows." } }
                form hx-post="/partials/events" hx-target="#items" hx-swap="innerHTML" {
                    input type="text" name="title" placeholder="Title" required;
                    textarea name="detail" placeholder="Details" required {}
                    button type="submit" { "Create Event" }
                }
                section id="items" { (content) }
                script { (maud::PreEscaped("const proto=location.protocol==='https:'?'wss':'ws';const ws=new WebSocket(proto+'://'+location.host+'/ws');ws.onmessage=()=>htmx.ajax('GET','/partials/events',{target:'#items'});")) }
            }
        }
    }
}
