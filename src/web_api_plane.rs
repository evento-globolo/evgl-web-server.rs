//! Four-avenue web ↔ API binding for `evento-globolo`.
//!
//! 1. Direct read-only DB via `*-lib-core` named queries (no migrations).
//! 2. Stateless HTTP from `app.evento-globolo.dev` to `api.evento-globolo.dev`.
//! 3. Stateful TLS 1.3/mTLS TCP to `api.evento-globolo.dev:7443`.
...
pub const GITHUB_ORG: &str = "evento-globolo";
pub const ORG_SLUG: &str = "evgl";
pub const DNS_ZONE: &str = "evento-globolo.dev";
